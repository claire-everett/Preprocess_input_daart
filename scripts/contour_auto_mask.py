...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 29 14:19:25 2022
...
video_dir = '/Users/claireeverett/Desktop/Process_input/videos/trim/'
...
class Args:
  ...
  shrink_kernel = 5 #the parameter in cv2.erode and cv2.dilate


def pre_filter(image,head_x,head_y):
    ''''Function that creates the dimensions of the masked image'''
    out = image.copy()
    l = out.shape[0]
    w = out.shape[1]
    # if head too close to side
    if head_x + 100 >= w:
        w_max = w
        w_min = w - 100
    elif head_x - 100 < 0:
    ...
    out[:int(l_min),:] = 255 # top 
    out[int(l_max):,:] = 255 # bottom
    out[:, :int(w_min)] = 255 # left sdie
    out[:,int(w_max):] = 255 # right side
    return out
...
def find_contour(videopath,head,length=40*300,start=0,step=1,pre_filter=None,threshold = 50,shrink_kernel = 3):
    ...
    for i in tqdm(range(length)):
        ...
        if pre_filter is not None:
            ...
            image = pre_filter(image, head_x, head_y)
        # plt.figure()
        # plt.imshow(image)
        index+=step
        vidcap.set(1,index)
...
# then make mask for each frame based on the location of the head marker

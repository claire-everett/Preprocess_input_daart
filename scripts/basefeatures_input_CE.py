...
for i in np.arange(len(DLC_paths)):
    ...
    df_filt.to_csv(os.path.join(export_dir,os.path.basename(DLC_paths[i]).split('_')[1] + '_labeled.csv' ))
    
# else:
#     print('doesnt match_location')
# else: 
# print('doesnt match')
        
        #%%
# smooth all columns of the basefeatures
...
for i in new_files:
    ...
    df_filt.to_csv(os.path.join(export_dir, os.path.basename(i).split('pi')[0] + '_labeled.csv'))

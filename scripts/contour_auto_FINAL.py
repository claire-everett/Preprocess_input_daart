...
def find_background(video_path,sample_length = None,image_shape = (500,500,3)):
    ...
    while True:
        ret, frame = cap.read()
        #cv2.imwrite('/Users/claireeverett/Desktop/frame.png', frame)
        n_sample += 1
    ...
    plt.show()
    cv2.imwrite('/Users/claireeverett/Desktop/background.png', frames)
    return frames

def pre_filter(background, frame):
    # get background mask
    background = cv2.imread(background)
    imgray_back = cv2.cvtColor(background, cv2.COLOR_BGR2GRAY)
    ret, thresh_back = cv2.threshold(imgray_back, 50, 255, 0)
    thresh_back_flip = abs(thresh_back.astype(float) - 255)
    # get the frame mask
    imgray_fore = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    ret, thresh_fore = cv2.threshold(imgray_fore, 50, 255, 0)
    thresh_fore_flip = abs(thresh_fore.astype(float) - 255)
    # cv2.imwrite('/Users/claireeverett/Desktop/function_test_fore_new.png', thresh_fore_flip)

    subtraction  = cv2.subtract(thresh_fore_flip, thresh_back_flip)

    cv2.imwrite('/Users/claireeverett/Desktop/subtraction.png', subtraction)
    
    im = cv2.imread('/Users/claireeverett/Desktop/subtraction.png')
    return im
...
def find_contour(videopath,head,length=40*300,start=0,step=1,pre_filter=None,threshold = 50,shrink_kernel = 3):
    ...
    '''
    background = find_background(videopath, sample_length = 5000, image_shape = (500, 500, 3))
    ...
    for i in tqdm(range(length)):
        success,image_first=vidcap.read()
        if success!=1:
            print("process stops early at {}th iteration".format(i))
            break
        if pre_filter is not None:            
            image = np.invert(pre_filter('/Users/claireeverett/Desktop/background.png', image_first))
        #plt.figure()
        #plt.imshow(image)
        index+=step
        vidcap.set(1,index)
        ...
        if first: #plot sample contour 
            sample_image = cv2.drawContours(image_first.copy(),[true_contour],0,(255,0,0),2)
...
video_dir = '/Users/claireeverett/Desktop/Process_input/new_contour_script/'
...
class Args:
  video_path = None
  h5_path = None
  output_root = '/Users/claireeverett/Desktop/Process_input/new_contour_script/'
